...
const App = () => {
  ...
  // state: movies
  const [appMovies, setAppMovies] =
    useState([]);
  const [
    allMoviesOptionGender,
    setAllMoviesOptionGender,
  ] = useState("");
  const [
    allMoviesOptionSort,
    setAllMoviesOptionSort,
  ] = useState("asc");
  ...
  */
  const sendProfileToApi = (
    userId,
    data
  ) => {
    apiUser
      .sendProfileToApi(userId, data)
      .then(() => {
